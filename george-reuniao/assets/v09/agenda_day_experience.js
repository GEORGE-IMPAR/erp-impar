...
function openImage(url){
 let dlg=$('georgeMediaViewer');if(!dlg){dlg=document.createElement('dialog');dlg.id='georgeMediaViewer';dlg.className='george-media-viewer';const close=document.createElement('button');close.type='button';close.className='g09-button';close.textContent='Fechar';close.onclick=()=>dlg.close();const image=document.createElement('img');image.alt='Foto ampliada';dlg.append(close,image);dlg.addEventListener('click',event=>{if(event.target===dlg)dlg.close();});document.body.append(dlg);}dlg.querySelector('img').src=url;if(!dlg.open)dlg.showModal();
}
function makePreview(host,file,kind){
 host.className='media-card-media';
 if(file&&(kind==='image'||kind==='video')){const url=URL.createObjectURL(file),node=document.createElement(kind==='image'?'img':'video');node.src=url;if(kind==='image'){node.alt='Foto anexada';node.tabIndex=0;node.setAttribute('role','button');node.setAttribute('aria-label','Ampliar foto');node.onclick=()=>openImage(url);node.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openImage(url);}};}else{node.controls=true;node.playsInline=true;node.preload='metadata';node.setAttribute('aria-label','Reproduzir vídeo anexado');const play=document.createElement('button');play.type='button';play.className='media-card-play';play.setAttribute('aria-label','Reproduzir vídeo');play.textContent='▶';play.onclick=()=>node.play();node.addEventListener('playing',()=>{play.hidden=true;});node.addEventListener('pause',()=>{if(!node.ended)play.hidden=false;});node.addEventListener('ended',()=>{play.hidden=false;});host.append(node,play);return url;}host.append(node);return url;}
...
function create(file,forcedKind){
 ...
 const api={row,kind,file,url,id:null,setId(id){this.id=id;row.dataset.recordId=id;cards.set(id,this);return this;},source(blob,name){if(!(blob instanceof Blob))return null;if(url)URL.revokeObjectURL(url);mediaHost.replaceChildren();const next=blob instanceof File?blob:new File([blob],name||labelOf(kind),{type:blob.type});this.file=next;url=makePreview(mediaHost,next,kind);this.url=url;return next;},update(label,percent=null){status.querySelector('strong').textContent=label;const numeric=Number.isFinite(percent);progress.classList.toggle('indeterminate',!numeric);status.querySelector('span').textContent=numeric?Math.round(percent)+'%':'';if(numeric)progress.querySelector('b').style.width=Math.max(0,Math.min(100,percent))+'%';row.classList.remove('media-card-error');},job(job){
