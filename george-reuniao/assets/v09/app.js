...
const state={csrf:'',user:null,record:null,mode:'text',busy:false,recording:null,auth:false,upload:false,jobRunning:new Set(),logQueue:Promise.resolve(),chatQueue:Promise.resolve(),caps:null,activeModule:'geral',lastDocument:null,lastAnalytics:null,lastAgendaReport:null,lastAgendaReportError:'',lastOutput:null,lastWakeAt:0};
...
$('loginForm').addEventListener('submit',async e=>{
 ...
 try{const j=await request('login',{email:$('loginEmail').value,password:$('loginPassword').value});$('loginPassword').value='';state.csrf=j.csrf;state.user=j.user;state.auth=true;$('loginDialog').close();await initialize({newConversation:true});}
...
function normalizeGeorgeCall(text){
 ...
 const start=new RegExp('^((?:ei|oi|ola|o|opa)[, .:;-]*)?('+georgeName+')\\b','i');
 const end=new RegExp('\\b('+georgeName+')[, .:;!?-]*$','i');
 return raw.replace(start,(_,prefix)=>`${prefix||''}GEORGE`).replace(end,'GEORGE');
}
function agendaReportIntent(text){
 if(state.activeModule!=='agenda_dia'||state.recording)return false;
 const n=normalized(text).replace(new RegExp('\\b'+georgeName+'\\b','g'),' ');
 if(/\b(por que|porque|motivo|erro|falhou|falha|problema|nao conseguiu|nao consegue)\b/.test(n)&&/\b(pdf|relatorio)\b/.test(n))return false;
 const report=/\b(relatorio|pdf)\b/.test(n)&&/\b(gera|gerar|gere|monte|montar|crie|criar|quero|preciso|arquivo|compartilha|compartilhar|baixa|baixar|imprime|imprimir|exporta|exportar)\b/.test(n);
...
const voice={pc:null,dc:null,stream:null,sender:null,audio:null,live:false,connecting:null,speaking:false,speakDone:null,seen:new Set(),wanted:false,followupUntil:0,reconnectTimer:null,reconnectAttempt:0,
 ...
 async connect(){
   ...
   this.connecting=(async()=>{
    ...
    this.wanted=true;this.followupUntil=Date.now()+20000;this.resumeTransmit();normalStatus();
   })();
...
transcriptOrder:[],transcriptFinals:new Map(),transcriptCommittedAt:new Map(),transcriptTimer:null,
 ...
 acceptTranscript(e){
    const text=normalizeGeorgeCall(String(e.transcript||'').trim()),id=e.item_id||eventId();
 ...
 async speak(text,full=false,preservePaused=false){
   if(state.mode==='text'&&!state.recording)return;
   const retainedPause=preservePaused?this.pausedSpeech:null;
   await this.connect();this.stopSpeaking(false);this.pausedSpeech=retainedPause;
   ...
   finally{if(ticket===this.speechTicket){this.speaking=false;this.tts=null;this.ttsAbort=null;this.speakDone=null;this.markSpeaking(false);this.followupUntil=Date.now()+20000;this.resumeTransmit();if(preservePaused&&retainedPause)this.pausedSpeech=retainedPause;normalStatus();}}
 },
...
async function reviewMeeting(){
 ...
 try{const r=state.recording,j=await request('meeting_review',{record_id:r.id},45000);if(state.recording!==r||r.stopping)return;meetingProposal=j.question&&['offered','deferred'].includes(j.question.state)?j.question:null;if(!j.proposal)return;
  const m=message('george','Gente, eu tenho uma dúvida sobre o que foi discutido. Posso falar?','REUNIÃO');
  ...
  if(!voice.speaking&&!state.busy)await voice.speak('Gente, eu tenho uma dúvida sobre o que foi discutido. Posso falar?');
...
function handlePauseInterruption(text,source,id){
 ...
 message('me',text);
 if(state.recording?.mode==='meeting'&&meetingProposal){
...
function handleLocalIntent(text,source,id){
 ...
 if(isDoc&&!window.GeorgeAgendaReport?.matches?.(text,state.activeModule)&&!agendaReportIntent(text)){
  ...
  state.chatQueue=state.chatQueue.catch(()=>{}).then(async()=>{const indicator=notice('Pedido recebido • preparando o PDF…');const operation=window.GeorgeAgendaExperience?.operation('Gerando PDF','Reunindo as fontes da conversa…');try{await state.logQueue;
...
async function initialize(options={}){
 try{
  if(options.newConversation)await request('conversation_new');
  const j=await request('resume');state.record=j.record_id;state.activeModule=j.module||'geral';state.lastDocument=null;state.lastAnalytics=null;state.lastAgendaReport=null;state.lastAgendaReportError='';state.lastOutput=null;input.value=j.draft||'';autosize();
 ...
 if(new URLSearchParams(location.search).has('diagnostico')){const n=message('george','Diagnóstico de instalação','DIAGNÓSTICO');const pre=document.createElement('pre');pre.className='diagnostic';pre.textContent=JSON.stringify({...h,frontend_version:'0.9.8-rc3-hf7-video-validacao'},null,2);n.bubble.append(pre);state.mode='text';normalStatus();return;}
  ...
  state.mode='text';setActive('');normalStatus();followChat=true;scroll(true);
 }catch(e){error(e);}
}
setupConversationTools();
(async()=>{try{setActive('');await (window.GeorgePwaSplashReady||Promise.resolve());if(await authenticate(false))await initialize({newConversation:true});}catch(e){error(e);}})();
})();
