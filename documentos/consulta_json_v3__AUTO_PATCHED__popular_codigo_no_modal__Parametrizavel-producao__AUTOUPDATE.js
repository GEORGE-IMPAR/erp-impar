...
// === Autoatualizar documento ao clicar no código ===
(function(){
  var oldOpenDecide = window.openDecide;
  if (typeof oldOpenDecide !== 'function') return;

  window.openDecide = function(code){
    try { oldOpenDecide(code); } catch(e) { try { console.error('openDecide original falhou:', e); } catch(_){ } }
    // pequeno delay para garantir que o card foi montado
    setTimeout(function(){
      try{
        var btnAtualizar = document.getElementById('cj_btn_atualizar');
        if (btnAtualizar && typeof btnAtualizar.click === 'function') {
          btnAtualizar.click();
        } else {
          // fallback: dispara diretamente a rotina interna, caso o botão não exista
          try {
            var chip = document.getElementById('cj_code_chip');
            var code2 = (chip && chip.getAttribute('data-code') || '').trim() || (code||'').trim();
            if (code2){
              var inp = document.getElementById('codigo'); if (inp) inp.value = code2;
              if (typeof fetchDoc === 'function'){
                fetchDoc(code2).then(function(item){
                  try { if (typeof fillForm === 'function') fillForm(item); } catch(_){}
                  try { if (typeof goTo === 'function') goTo(2); } catch(_){}
                  try { 
                    var b1 = document.getElementById('cj_back1');
                    var b2 = document.getElementById('cj_back2');
                    if (b1) b1.style.display='none';
                    if (b2) b2.style.display='none';
                  } catch(_){}
                  window.scrollTo({top:0,behavior:'smooth'});
      ...
      }catch(err){ try{ console.error('Erro ao autoatualizar:', err); }catch(_){ } }
    }, 300);
  };
})();
