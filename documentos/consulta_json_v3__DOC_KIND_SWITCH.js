...
q('cj_btn_gerar').onclick = async function () {
  ...
  async function gerarContratoOnce(c) {
    try {
      const res = await fetch((window.__DOC_KIND__ && window.__DOC_KIND__.endpoint ? window.__DOC_KIND__.endpoint : '/api/gerador/make_contract.php')+'?codigo=' + encodeURIComponent(c), { cache: 'no-store' });
