...
/* consulta_json_v3_beauty_brand_fixclose.js
   Igual à beauty_brand, mas garante que NENHUMA janela antiga reapareça ao fechar.
   - hideLegacy(): esconde modais antigos por IDs conhecidos e por heurística (título "Consulta de documentos (Excel)")
*/
(function(){
  ...
  function hideLegacy(){
    // IDs/classes que vimos em versões antigas
    ...
    // Heurística: qualquer container que contenha o título antigo
    try{
      ...
      for (var i=0;i<nodes.length;i++){
        ...
        if (!txt) continue;
        if (txt.indexOf('Consulta de documentos (Excel)') !== -1){
  ...
  function injectCSS(){
    ...
    var css = [
      ...
      '.cj-code{font-weight:800;color:'+BRAND.primary+'}', '.cj-date{color:#475569;font-size:13px}', '.cj-client{color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
      ...
      '.btn{border:none;border-radius:999px;padding:12px 16px;cursor:pointer;font-weight:800;letter-spacing:.2px}', '.btn.ghost{background:#e2e8f0;color:'+BRAND.primary+'}', '.btn.primary{background:linear-gradient(90deg,'+BRAND.primaryDark+','+BRAND.primary+');color:#fff}'
    ].join('');
  ...
  function build(){
    ...
    box.innerHTML = '<div class="cj-head"><div class="cj-title">Consulta de documentos</div><button class="cj-x" id="cj_x1">×</button></div>'
    ...
    card.innerHTML = '<div class="cj-card-head"><div class="cj-title">Documento <span id="cj_code_chip" class="cj-chip">—</span></div><button class="cj-x" id="cj_x2">×</button></div>'
                   + '<div class="cj-card-body">O que você deseja fazer com este documento?</div>'
                   + '<div class="cj-actions"><button class="btn ghost" id="cj_btn_close">Fechar</button><button class="btn ghost" id="cj_btn_gerar">Gerar contrato</button><button class="btn primary" id="cj_btn_atualizar">Atualizar documento</button></div>';
    b2.appendChild(card); document.body.appendChild(b2);

    function hideAll(){ b1.style.display='none'; b2.style.display='none'; hideLegacy(); }
    q('cj_x1').onclick=hideAll; q('cj_x2').onclick=hideAll; q('cj_btn_close').onclick=hideAll; q('cj_btn_gerar').onclick=function(){ b2.style.display='none'; hideLegacy(); };

    q('cj_btn_atualizar').onclick=function(){
      var code=(q('cj_code_chip').getAttribute('data-code')||'').trim();
      if(!code){ hideAll(); return; }
      var inp=q('codigo');
      if(inp){ inp.value=code; try{ inp.dispatchEvent(new Event('input',{bubbles:true})); inp.dispatchEvent(new Event('change',{bubbles:true})); }catch(e){} }
      fetchDoc(code).then(function(item){ fillForm(item); hideAll(); window.scrollTo({top:0,behavior:'smooth'}); }).catch(function(){ hideAll(); });
    };

    window.__CJFIX__={b1:b1,b2:b2};
  }

  function openList(){
    build();
    hideLegacy();
    window.__CJFIX__.b2.style.display='none'; window.__CJFIX__.b1.style.display='flex';
  }
  function openDecide(code){
    build();
    q('cj_code_chip').textContent=code; q('cj_code_chip').setAttribute('data-code', code);
    window.__CJFIX__.b1.style.display='none'; window.__CJFIX__.b2.style.display='flex';
    hideLegacy();
  }
