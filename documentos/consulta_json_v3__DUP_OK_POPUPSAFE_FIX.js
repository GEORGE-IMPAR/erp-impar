...
(function(){
  ...
  function build(){
    ...
    /* ATENÇÃO: deixe os rótulos como preferir;
       o importante é que o clone tenha type="button" */
    card.innerHTML='<div class="cj-card-head"><div class="cj-title">Documento <span id="cj_code_chip" class="cj-chip">—</span></div><button class="cj-x" id="cj_x2">×</button></div><div class="cj-card-body">O que você deseja fazer com este documento?</div><div class="cj-actions"><button class="btn ghost" id="cj_btn_close">Fechar</button><button class="btn ghost" id="cj_btn_gerar-1" type="button">Gerar contrato</button><button class="btn ghost" id="cj_btn_gerar">Gerar contrato -1</button><button class="btn primary" id="cj_btn_atualizar">Atualizar documento</button></div>';
...
/* --- Duplicado 1:1 (handler do botão Gerar contrato -1) — POPUP SAFE --- */
q('cj_btn_gerar-1').onclick = async function () {
  try { if (typeof event!=='undefined' && event) { event.preventDefault(); event.stopPropagation(); } } catch(_) {}
  const popup = window.open('about:blank', '_blank');

  var code = (q('cj_code_chip')?.getAttribute('data-code') || '').trim();
  if (!code) { try { __forceCloseConsultaUI && __forceCloseConsultaUI(); } catch (_) {} try { popup && popup.close(); } catch(_){} return; }
  ...
  async function gerarContratoOnce(c) {
    try {
      const url = '/api/gerador/make_contract.php?codigo=' + encodeURIComponent(c);
      const res = await fetch(url, { cache: 'no-store' });
      const j = await res.json();
      if (j && j.ok && j.url) {
        try {
          if (popup) { popup.location = j.url; } else { window.open(j.url, '_blank'); }
        } catch(_) {
          window.open(j.url, '_blank');
        }
        try {
      ...
      console.error('Erro ao gerar contrato (Botão 2):', e);
    }
    return false;
  }
  ...
  window.alert = originalAlert;
  hideLoader();
  if (!ok && popup) { try { popup.close(); } catch(_){} }
  try { __forceCloseConsultaUI && __forceCloseConsultaUI(); } catch (_) {}
};
...
window.__CJFIX__ = { b1:b1, b2:b2, loaderBack:lback };
  ...
  (function(){
    try {
      ...
      // Mantém o chip do modal coerente (idempotente; não quebra se já existir em outro ponto)
      ...
      // (Opcional) espelhos do código em spans/labels que seguem padrão "codigoVal*"
      try {
