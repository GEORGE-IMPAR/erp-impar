...
(function(){
  ...
  // NÃO preencher #codigo nem disparar input/change aqui.

  // Loader (se existir)
  try{ if (typeof lback!=='undefined' && lback) lback.style.display='flex'; }catch(_){}

  fetch('/api/gerador/make_contract.php?codigo='+encodeURIComponent(code))
    .then(function(r){ return r.json(); })
    .then(function(j){
      try{ if (typeof lback!=='undefined' && lback) lback.style.display='none'; }catch(_){}
      if(!j||!j.ok||!j.url){ hideAll(); return; }

      // Abrir/baixar contrato
      try{ window.open(j.url, '_blank'); }catch(_){}

      // Mostrar o mesmo modal de sucesso do index
      try{
        var sm=document.getElementById('successModal');
        if (sm){ sm.style.display='flex'; sm.style.visibility='visible'; sm.style.opacity='1'; }
      }catch(_){}

      // Reset leve + volta para etapa 1
      try{
        document.querySelectorAll('input[type=\"text\"], input[type=\"file\"], input[type=\"number\"], textarea, select')
          .forEach(function(el){
