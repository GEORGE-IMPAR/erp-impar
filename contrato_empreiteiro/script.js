...
async function getJson(url) {
  ...
  if (!res.ok) {
    throw new Error(
      `API indisponível (HTTP ${res.status}${res.statusText ? " " + res.statusText : ""})` +
      (txt ? ` — ${txt.slice(0, 300)}` : " — resposta vazia do servidor")
    );
  }
  ...
  try {
    ...
    throw new Error(`Resposta inválida da API: ${txt.slice(0, 300)}`);
  }
}
...
async function abrirContratosSalvos() {
  ...
  try {
    const js = await getJson(`${API_BASE}/listar_contratos.php?t=${Date.now()}`);
    ...
    console.error(e);
    body.innerHTML = `<tr><td colspan="6">Erro ao carregar contratos salvos: ${String(e.message || e)}</td></tr>`;
    Swal.fire({
      icon: "error",
...
async function carregarContratoSalvo(id) {
  try {
    const js = await getJson(`${API_BASE}/obter_contrato.php?id=${encodeURIComponent(id)}&t=${Date.now()}`);
...
async function carregarEmpreiteiros() {
  try {
    const js = await getJson(`${API_BASE}/listar_empreiteiros.php?t=${Date.now()}`);
    const sel = document.getElementById("empreiteiro");
...
async function carregarObras() {
  try {
    const js = await getJson(`${API_OBRAS}?t=${Date.now()}`);
