...
// ================== SSO ERP ÍMPAR (forçar usuário do MENU) ==================
const ERP_SESSION_KEY = "ERPIMPAR_USER";

// Retorna objeto do ERP (menu) ou null
function getSessaoERP() {
  try { return JSON.parse(localStorage.getItem(ERP_SESSION_KEY) || "null"); }
  catch(e) { return null; }
}

// Limpa apenas a sessão do MÓDULO (não mexe no ERPIMPAR_USER)
function limparSessaoModuloMateriais() {
  try {
...
// Gera o "usuarioLogado" no formato esperado pelo módulo, SEMPRE a partir do ERP
function syncUsuarioDoMenuParaModulo() {
  const erp = getSessaoERP();
  if (!erp || !erp.email) return null;

  const nome = erp.nome || erp.Nome || erp.name || erp.email || "";
  const primeiro = (String(nome).trim().split(" ")[0] || "");
  const responsavel = (primeiro || (erp.email.split("@")[0] || "")).toUpperCase();
  ...
  // Força overwrite (evita manter "Daniel" ou usuário antigo)
  try {
...
// No carregamento do módulo, sempre sincroniza usuário do menu
(function(){
  // Se não tem sessão do ERP, manda para o INDEX (login único)
  const erp = getSessaoERP();
  if (!erp || !erp.email) {
    limparSessaoModuloMateriais();
    window.location.replace("index.html");
    return;
  }

  // sempre força o usuário do menu para dentro do módulo
  syncUsuarioDoMenuParaModulo();

  // Ao sair do módulo (fechar aba/atualizar/navegar), limpa a sessão local do módulo
...
function ensureUsuarioLogadoSSO(){
  return syncUsuarioDoMenuParaModulo();
}
catch (e) {}
