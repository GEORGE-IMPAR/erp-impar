...
function ensureUsuarioLogadoSSO() {
  // Fonte única de verdade: sessão global do menu (SSO leve)
  const sso = getSessaoERP();
  if (!sso || (!sso.email && !sso.Email)) return null;

  const ssoEmail = String(sso.email || sso.Email || "").trim().toLowerCase();
  if (!ssoEmail) return null;

  // Estrutura esperada pelo módulo de Materiais (compatível com script original)
  const usuarioSSO = {
    Nome: sso.nome || sso.Nome || sso.name || ssoEmail.split("@")[0],
    Email: (sso.email || sso.Email || "").trim(),
    Cargo: sso.cargo || sso.Cargo || ""
  };

  // Se existir um usuarioLogado antigo, mas for de outro email, sobrescreve (evita "Daniel" preso)
  try {
    const raw = localStorage.getItem("usuarioLogado");
    if (raw) {
      const atual = JSON.parse(raw);
      const atualEmail = String(atual?.Email || atual?.email || "").trim().toLowerCase();
      if (atualEmail && atualEmail === ssoEmail) {
        // mantém o mesmo objeto, mas garante campos principais
        localStorage.setItem("usuarioLogado", JSON.stringify({ ...atual, ...usuarioSSO }));
        return { ...atual, ...usuarioSSO };
      }
    }
  } catch (e) {}

  localStorage.setItem("usuarioLogado", JSON.stringify(usuarioSSO));
  return usuarioSSO;
}
