(function(global){
  ...
  function mergeHomes(cars){
    const items=[],spreadsheetPlates=new Set(cars.filter(car=>car.enderecoResidencial).map(car=>car.placa));
    cars.forEach(car=>{
      if(car.enderecoResidencial){
        items.push({placa:car.placa,nome:car.responsavel||'',endereco:car.enderecoResidencial,confirmado:true,principal:driverByPlate[car.placa]===String(car.responsavel||'').trim().toUpperCase(),origem:'Planilha'});
        return;
      }
    ...
    loadSavedHomes().forEach(item=>{
      ...
      if(spreadsheetPlates.has(item.placa))return;
      const index=items.findIndex(x=>x.placa===item.placa&&normName(x.nome)===normName(item.nome));
      if(index>=0){
        if(items[index].origem!=='Planilha')items[index]={...items[index],...item,confirmado:true,origem:'Cadastro salvo'};
      }
