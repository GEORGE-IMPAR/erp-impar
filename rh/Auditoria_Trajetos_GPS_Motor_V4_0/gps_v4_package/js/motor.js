(function(global){
  ...
  function buildCycles(events,initialAudit=[]){
    ...
    const orderedStreams=[...streams.values()].sort((a,b)=>(a[0].dt-b[0].dt)||String(a[0].plate).localeCompare(String(b[0].plate),'pt-BR'));

    for(const stream of orderedStreams){
      stream.sort((a,b)=>(a.dt-b.dt)||(a.rawIndex-b.rawIndex));
      let current=null,pendingOff=null;
      ...
      function closeCurrent(incomplete){
        if(!current)return;
        if(pendingOff){
          current.points.push(pendingOff);current.end=pendingOff;current.incomplete=false;
      ...
      for(const event of stream){
        ...
        if(isOn(event.type)){
          ...
          if(last&&isOn(last.type)&&Core.norm(last.address)===Core.norm(event.address)){
            current.points[current.points.length-1]=event;current.discarded++;
  ...
  function buildSegments(cycles){
    ...
    cycles.forEach(cycle=>{
      for(let i=0;i<cycle.points.length-1;i++){
        const a=cycle.points[i],b=cycle.points[i+1];
        if(Core.norm(a.address)===Core.norm(b.address))continue;
        n++;segments.push({id:`T${String(n).padStart(4,'0')}`,cycleId:cycle.id,plate:cycle.plate,date:cycle.date,departure:a.dt,origin:a.address,arrival:b.dt,destination:b.address,duration:b.dt-a.dt,km:null});
      }
    });
    return segments;
  }

  function buildStays(cycles){
    const stays=[];let n=0;
    const groups=new Map();
    for(const cycle of cycles){
      const key=`${cycle.plate}|${Core.iso(cycle.date)}`;
      if(!groups.has(key))groups.set(key,[]);
      groups.get(key).push(cycle);
    }
    for(const group of groups.values()){
      group.sort((a,b)=>a.start.dt-b.start.dt);
      for(let i=0;i<group.length-1;i++){
        const current=group[i],next=group[i+1];
        if(!current.end||next.start.dt<=current.end.dt)continue;
        n++;
        stays.push({id:`P${String(n).padStart(4,'0')}`,plate:current.plate,date:current.date,afterCycleId:current.id,beforeCycleId:next.id,start:current.end.dt,end:next.start.dt,duration:next.start.dt-current.end.dt,address:current.end.address||'',reason:'INTERVALO_ENTRE_CICLOS'});
      }
    }
    stays.sort((a,b)=>(a.start-b.start)||a.plate.localeCompare(b.plate,'pt-BR'));
    return stays;
  }

  function run(events,options={}){
    const pre=preprocess(events,options);
    const built=buildCycles(pre.accepted,pre.auditTrail);
    const segments=buildSegments(built.cycles);
    const stays=buildStays(built.cycles);
    return {received:pre.ordered,valid:pre.accepted,cycles:built.cycles,segments,stays,auditTrail:built.auditTrail};
  }
