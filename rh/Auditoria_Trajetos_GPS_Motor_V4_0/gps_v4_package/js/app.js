(function(global){
  'use strict';
  const {Core,Importer,Normalizer,Engine,UI,Financeiro}=global.GPSV4;const $=id=>document.getElementById(id);let base=[],lastResult=null,mode='todos',metadata={cadastroCarros:[],cadastroObras:[],sede:null};
  ...
  function updateFinance(){if(!lastResult)return;UI.renderFinanceiro(Financeiro.run(lastResult,metadata,params()));global.GPSV4.MapUI?.renderLocations(metadata,$('raio').value)}
  ...
  $('arquivo').addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;UI.status('Lendo e normalizando arquivo...');try{const imported=await Importer.readFile(file);metadata={...(Importer.readFile.lastMetadata||{cadastroCarros:[],cadastroObras:[]}),sede:null};base=Normalizer.run(imported).eventos;if(!base.length)throw new Error('Não encontrei registros válidos. Confirme os cabeçalhos Data/Hora e Rastreável.');mode='todos';document.querySelectorAll('.chip').forEach(chip=>chip.classList.toggle('active',chip.dataset.mode==='todos'));UI.fillFilters(base);$('localizarPontos').disabled=false;$('atualizarFinanceiro').disabled=false;process()}catch(error){console.error(error);UI.status(error.message||'Falha ao ler o arquivo.','error')}});
