(function(global){
  ...
  $('arquivo').addEventListener('change',async e=>{const file=e.target.files[0];if(!file)return;UI.status('Lendo e normalizando arquivo...');try{const imported=await Importer.readFile(file);metadata={...(Importer.readFile.lastMetadata||{cadastroCarros:[],cadastroObras:[]}),sede:null,residencias:[{placa:'IZH-2A86',endereco:'Rua Antônio de Paula Xavier, Prado de Baixo, Biguaçu, SC',confirmado:true},{placa:'RXO-8A58',endereco:'Rua Irineu Pavan, Barreiros, São José, SC',confirmado:true}]};locationsReady=false;base=Normalizer.run(imported).eventos;inferNightAnchors(base,metadata.residencias);if(!base.length)throw new Error('Não encontrei registros válidos. Confirme os cabeçalhos Data/Hora e Rastreável.');mode='todos';document.querySelectorAll('.chip').forEach(chip=>chip.classList.toggle('active',chip.dataset.mode==='todos'));UI.fillFilters(base);$('localizarPontos').disabled=false;$('atualizarFinanceiro').disabled=false;process();await locatePoints(true)}catch(error){console.error(error);UI.status(error.message||'Falha ao ler o arquivo.','error')}});
  ...
  async function locatePoints(automatic=false){
    ...
    try{
      ...
      const targets=[{tipo:'sede',nome:'Empresa',endereco:$('enderecoSede').value},...metadata.cadastroObras.map(x=>({tipo:'obra',ref:x,nome:x.nome,endereco:x.endereco})),...metadata.residencias.filter(x=>x.endereco).map(x=>({tipo:'residencia',ref:x,nome:`Residência operacional ${x.placa}`,endereco:x.endereco}))];
