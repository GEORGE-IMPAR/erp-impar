(function(global){
  ...
  const classificationMeta={
    obra:{label:'3.1 Permanência em obra',className:'obra'},
    empresa:{label:'3.2 Permanência/trecho na empresa',className:'empresa'},
    residencia_empresa:{label:'3.3 Residência → empresa',className:'ida'},
    empresa_residencia:{label:'3.4 Empresa → residência',className:'volta'},
    empresa_obra:{label:'3.5 Empresa → obra',className:'obra'},
    obra_empresa:{label:'3.6 Obra → empresa',className:'obra'},
    residencia_obra:{label:'3.7 Residência/hotel → obra',className:'ida'},
    obra_residencia:{label:'3.8 Obra → residência/hotel',className:'volta'},
    fim_semana:{label:'1. Sábado e domingo',className:'fim-semana'},
    fora_horario:{label:'2. Fora do horário',className:'fora-horario'},
    rota_nao_prevista:{label:'4. Rota não prevista',className:'nao-classificado'}
  };
  ...
  UI.renderFinanceiro=data=>{
    ...
    const obraTotal=['obra','empresa_obra','obra_empresa','residencia_obra','obra_residencia'].reduce((a,key)=>({km:a.km+s[key].km,custo:a.custo+s[key].custo}),{km:0,custo:0});
    bind('fObras','fObrasCusto',obraTotal);bind('fEmpresa','fEmpresaCusto',s.empresa);
    bind('fIda','fIdaCusto',s.residencia_empresa);bind('fVolta','fVoltaCusto',s.empresa_residencia);
    bind('fEmpresaObra','fEmpresaObraCusto',s.empresa_obra);bind('fObraEmpresa','fObraEmpresaCusto',s.obra_empresa);
    bind('fResidenciaObra','fResidenciaObraCusto',s.residencia_obra);bind('fObraResidencia','fObraResidenciaCusto',s.obra_residencia);
    bind('fFimSemana','fFimSemanaCusto',s.fim_semana);bind('fForaHorario','fForaHorarioCusto',s.fora_horario);
    bind('fNaoClassificado','fNaoClassificadoCusto',s.rota_nao_prevista);
    const labels={obra:'3.1. Permanência em obra',empresa:'3.2. Empresa',residencia_empresa:'3.3. Residência → empresa',empresa_residencia:'3.4. Empresa → residência',empresa_obra:'3.5. Empresa → obra',obra_empresa:'3.6. Obra → empresa',residencia_obra:'3.7. Residência/hotel → obra',obra_residencia:'3.8. Obra → residência/hotel',fim_semana:'1. Sábado e domingo',fora_horario:'2. Fora do horário (21:00–05:30)',rota_nao_prevista:'4. Rota não prevista'};
    const categories=['fim_semana','fora_horario','residencia_empresa','empresa_residencia','empresa_obra','obra_empresa','residencia_obra','obra_residencia','obra','empresa','rota_nao_prevista'];
    ...
    $('financeiroAviso').textContent=`Cálculo: ${s.kmLitro.toLocaleString('pt-BR')} km/L • ${money(s.precoLitro)}/L • raio ${s.raio.toLocaleString('pt-BR')} m • prioridade: fim de semana → 21:00–05:30 → rotas operacionais → rota não prevista. Obra confirmada após 40 min no raio.`;
  ...
  UI.financePending=message=>{
    ...
    ['fTotal','fObras','fEmpresa','fIda','fVolta','fEmpresaObra','fObraEmpresa','fResidenciaObra','fObraResidencia','fFimSemana','fForaHorario','fNaoClassificado'].forEach(id=>$(id).textContent='—');
    ['fTotalCusto','fObrasCusto','fEmpresaCusto','fIdaCusto','fVoltaCusto','fEmpresaObraCusto','fObraEmpresaCusto','fResidenciaObraCusto','fObraResidenciaCusto','fFimSemanaCusto','fForaHorarioCusto','fNaoClassificadoCusto'].forEach(id=>$(id).textContent='Aguardando localização');
