(function(global){
  ...
  UI.renderCadastros=metadata=>{
    ...
    $('tbodyCarros').innerHTML=carros.length?carros.map(x=>{const r=residencias.find(item=>item.placa===x.placa)||{};const status=r.confirmado?'<span class="badge ok">Confirmado</span>':r.regraUsuario?`<span class="badge info">Regra operacional • ${r.noites} noite(s)</span>`:Number.isFinite(r.latitude)?'<span class="badge ok">Localizado</span>':'<span class="badge warn">Pendente</span>';return `<tr><td>${Core.escape(x.placa)}</td><td>${Core.escape(x.responsavel||'—')}</td><td>${Core.escape((x.equipe||[]).map(p=>p.nome).join(', ')||'—')}</td><td>R$ ${Number(x.valorHora||0).toFixed(2).replace('.',',')}</td><td>${Number(x.horasDia||0).toLocaleString('pt-BR')}</td><td><input class="residencia-endereco" data-placa="${Core.escape(x.placa)}" value="${Core.escape(r.endereco||'')}" placeholder="${r.regraUsuario?'Âncora noturna operacional — informe o endereço para confirmar':'Endereço residencial operacional'}"></td><td>${status}</td></tr>`}).join(''):'<tr><td colspan="7" class="empty">A aba Cadastro_Carros_Valor_h_h não foi encontrada.</td></tr>';
  ...
  UI.financePending=message=>{
    ...
    $('tbodyDeslocamentos').innerHTML='<tr><td colspan="7" class="empty">Aguardando classificação.</td></tr>';
  };
  ...
  UI.renderDeslocamentos=data=>{
    ...
    $('tbodyDeslocamentos').innerHTML=data.porPlaca.map(x=>`<tr><td>${Core.escape(x.placa)}</td><td>${Core.escape(x.responsavel||'—')}</td><td>${km(x.commuteKm)}</td><td>${km(x.weekendKm)}</td><td>${km(x.offhoursKm)}</td><td>${km(x.totalKm)}</td><td>${money(x.custo)}</td></tr>`).join('');
  };
