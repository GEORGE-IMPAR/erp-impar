(function(global){
  ...
  F.deslocamentos=(result,metadata,params)=>{
    ...
    const porPlaca=[...rows.values()].map(row=>{row.totalKm=row.commuteKm+row.weekendKm+row.offhoursKm;row.custo=row.totalKm/kmLitro*precoLitro;return row}).sort((a,b)=>b.totalKm-a.totalKm);
